...
Codex 顾问默认使用 `strict` 配置模式：从本轮 `0700` 私有空目录启动，以绝对路径在 prompt 中指向仓库与 discussion card；不得以仓库作为 cwd，也不向 CLI 授予额外目录。调用保留 `--ephemeral --sandbox read-only --ignore-user-config --ignore-rules --skip-git-repo-check`，同时固定官方 `openai` provider，关闭 project docs、bundled skill instructions、hooks、plugins、apps、browser/computer-use、memories、multi-agent、image generation、workspace dependency/skill discovery、shell snapshot 等可扩展工具面。wrapper 必须预检当前 CLI 支持全部安全参数和 feature 名；缺一项即 fail closed，不静默降级。read-only sandbox 约束该工具面中的写操作，但同一 UID 可全局读取的文件仍可能被读取，不能把它描述成仅仓库可读、机密隔离或 OS 级隔离；管理员/managed policy 也仍可生效。因此该模式只称“受限工具面并经 Git 审计的只读第二意见”。
